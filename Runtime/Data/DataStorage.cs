namespace DRG.Data
{
    ...
    public class DataStorage : IEnumerable<IDataRecord>, IDataStorage
    {
        ...
        private readonly Dictionary<string, DataRecordObject> recordsObject = new Dictionary<string, DataRecordObject>();
        ...
        {
            foreach (var record in recordsInt) yield return record.Value;
            foreach (var record in recordsBool) yield return record.Value;
            foreach (var record in recordsFloat) yield return record.Value;
            foreach (var record in recordsString) yield return record.Value;
            foreach (var record in recordsObject) yield return record.Value;
        }
        ...
        {
            if (recordsInt.TryGetValue(key, out var resultInt)) { record = resultInt; return true; }
            if (recordsBool.TryGetValue(key, out var resultBool)) { record = resultBool; return true; }
            if (recordsFloat.TryGetValue(key, out var resultFloat)) { record = resultFloat; return true; }
            if (recordsString.TryGetValue(key, out var resultString)) { record = resultString; return true; }
            if (recordsObject.TryGetValue(key, out var resultObject)) { record = resultObject; return true; }
            record = null;
            return false;
        }
